...
class TrainApis(viewsets.GenericViewSet, viewsets.mixins.ListModelMixin,
    ...
    def getTransfer(self, request, *args, **kwargs):
        ...
        min_price = 999999999;
        transfer1 = Transfer()
        flight1_set = Flight.objects.filter(city = departure,departdate=date)
        train1_set = Train.objects.filter(station__icontains=departure,departdate=date)
        ...
        for train1 in train1_set:
            ...
            train2_set = Train.objects.filter(station=arrival1,endstation__icontains=arrival,departdate=date1,departtime__gte = time1)
        ...
        for flight1 in flight1_set:
            ...
            date1 = flight1.arrivaldate
            time1 = flight1.arrivaltime
            train2_set = Train.objects.filter(station__icontains=arrival1,endstation__icontains=arrival,departdate=date1,departtime__gte = time1)
            train2_price = 999999999
            for train2 in train2_set:
                train2_price_set = TrainPriceList.objects.filter(owner=train2.id)
                for train2_price_temp in train2_price_set:
