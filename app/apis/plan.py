...
from app.models import AdminMessage, AppUser,Plan,Plan_Ser,Plan_Comp
...
class PlanApis(viewsets.GenericViewSet, viewsets.mixins.ListModelMixin,
    ...
    def deleteMyPlan(self,request, *args, **kwargs):
        ...
        return Response(True,status=status.HTTP_200_OK)
    #添加一个同行活动
    @action(methods=['POST'], detail=False, url_path='addMyComp')
    def addMyComp(self, request, *args, **kwargs):
        request_user = _permission.user_check(request)
        if request_user <= 0:
            return error_response(Error.NOT_LOGIN, 'Please login.', status=status.HTTP_403_FORBIDDEN)
        type = request.data.get('type')
        ...
        return Response(True, status=status.HTTP_200_OK)
    #添加一个同行活动
    @action(methods=['POST'], detail=False, url_path='getMyComp')
    def getMyComp(self,request, *args, **kwargs):
        request_user = _permission.user_check(request)
        if request_user <= 0:
            return error_response(Error.NOT_LOGIN, 'Please login.', status=status.HTTP_403_FORBIDDEN)
        plan_set = Plan_Comp.objects.filter(person1=request_user,person2 = request.data.get('pal'))
        result_list = []
        for plan in plan_set:
            # 根据content split
            content = plan.content
    ...
    # 删除一个同行活动
    @action(methods=['POST'], detail=False, url_path='deleteMyComp')
    def deleteMyComp(self, request, *args, **kwargs):
        request_user = _permission.user_check(request)
        if request_user <= 0:
            return error_response(Error.NOT_LOGIN, 'Please login.', status=status.HTTP_403_FORBIDDEN)
        type = request.data.get('type')
