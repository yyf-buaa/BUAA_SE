...
from app.serializers import UserSerializer, TravelSerializer, MessageSerializer, CompanionSerializer
...
class UserApis(viewsets.GenericViewSet, mixins.CreateModelMixin,
    ...
    def _retrieve(self, user, detail=False, request_user_id=None):
        ...
        data = serializer.data
        likes = Travel.objects.filter(owner=user).annotate(likes_count=Count('likes')).aggregate(Sum('likes_count'))['likes_count__sum']
        if likes is None:
            likes = 0
        data['likes'] = likes
    ...
    def subscribe(self, request, *args, **kwargs):
        ...
        if cancel:
            ...
            removed_messages = Message.objects.filter(owner=user, target_users__id__in=objs, type=settings.MESSAGE_TYPE_SUBSCRIBE)
    ...
    def join_companion(self, request, *args, **kwargs):
        ...
        if cancel:
            ...
            removed_messages = Message.objects.filter(owner=user, related_companion_id__in=obj_ids, type=settings.MESSAGE_TYPE_COMPANION)
            for msg in removed_messages:
                for target in msg.target_users.all():
                    target.unread_messages.remove(msg)
            removed_messages = Message.objects.filter(owner=user, related_companion_id__in=obj_ids, type=settings.MESSAGE_TYPE_COMPANION_QUIT)
            ...
            for companion in objs:
                Message.create_message(user, companion.owner, companion=companion, type=settings.MESSAGE_TYPE_COMPANION_QUIT)
            ...
            return response({'failed': failure}, status=status.HTTP_200_OK if not failure else status.HTTP_206_PARTIAL_CONTENT)
        else:
            failure = []
            for companion in Companion.objects.filter(id__in=ids):
                if companion.fellows.count() >= companion.capacity:
                    ...
                    msgs = Message.objects.filter(related_companion=companion, target_users=cowner, owner=user, type=settings.MESSAGE_TYPE_COMPANION_QUIT)
                    if msgs:
                        msgs = msgs.first()
                        msgs.unread_users.remove(cowner)
                    msgs = Message.objects.filter(related_companion=companion, target_users=cowner, owner=user, type=settings.MESSAGE_TYPE_COMPANION)
                    if msgs:
                        msgs = msgs.first()
                        msgs.time = date.now()
                        msgs.unread_users.add(cowner)
                    else:
                        msgs = Message.create_message(user, companion.owner, companion=companion, type=settings.MESSAGE_TYPE_COMPANION)
                        if msgs:
                            msgs.unread_users.add(cowner)
            return response({'failed': failure}, status=status.HTTP_200_OK if not failure else status.HTTP_206_PARTIAL_CONTENT)
