...
class rec_model(nn.Module):
    ...
    def __init__(self, user_max_dict, item_max_dict, embed_dim=32, fc_size=100):
        ...
        # --------------------------------- item channel ---------------------------------------------
        # item embeddings 嵌入层
        self.embedding_itemID = nn.Embedding(item_max_dict['itemID'], embed_dim)  # normally 32
        self.embedding_ipositionID = nn.Embedding(item_max_dict['ipositionID'], embed_dim)
        # 若要用tag进行推荐采取 mode='sum'

        # 全连接层1  1*64
        self.fc_itemID = nn.Linear(embed_dim, embed_dim)
        self.fc_ipositionID = nn.Linear(embed_dim, embed_dim)

        # 全连接层2  1*100
        self.fc_item_combine = nn.Linear(embed_dim * 2, fc_size)
        ...
        self.BN_ipositionID = nn.BatchNorm2d(1)
    ...
    def forward(self, user_input, item_input):
        ...
        ipositionID = item_input['ipositionID']

        if torch.cuda.is_available():
            userID, upositionID, itemID, ipositionID = \
                userID.to(device), upositionID.to(device), itemID.to(device), ipositionID.to(device)
        ...
        feature_ipositionID = self.BN_ipositionID(F.relu(self.fc_ipositionID(self.embedding_ipositionID(ipositionID))))

        # feature_movie B x 1 x 100
        feature_item = F.tanh(self.fc_item_combine(torch.cat([feature_itemID, feature_ipositionID], 3))) \
            .view(-1, 1, 100)
